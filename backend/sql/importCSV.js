...
const csvFilePath = 'c:\\Users\\Marco\\Downloads\\edited_database.txt';
      ...
      try {
        ...
        await db.query(
          `INSERT INTO TRANSACTION (transaction_id, customer_id, staff_id, date, time_received, price, name, service_type, addon, status) 
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
          [transactionCounter, customerId, 5, date, timeReceived, price, customerName, serviceType, addons, status]
        );
        
        console.log(`✅ TN${String(transactionCounter).padStart(2, '0')}: ${customerName} - ${serviceType} - ₱${price}`);
        imported++;
        transactionCounter++;
      } catch (error) {
        if (error.code === 'ER_DUP_ENTRY') {
